...
- [x] Explain the differences between `client-side routing` and `server-side routing`.
        Server-side routing allows the back end handles new pages requested by the user. 
        Client side routing allows the browser to handle new pages while the backend is only responsible for new data. 

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
        GET / CREATE / READ PUT / UPDATE DELETE / DELETE POST
